...
const FormulaConfig: React.FC = () => {
  ...
  const [roleOriginalValues, setRoleOriginalValues] = useState<
    Record<string, string>
  >({});
  const [ticketEditMode, setTicketEditMode] = useState(false);
  const [ticketEditValues, setTicketEditValues] = useState<
    Record<string, string>
  >({});
  const [ticketOriginalValues, setTicketOriginalValues] = useState<
    Record<string, string>
  >({});
  ...
  const handleRoleEditStart = () => {
    const vals = Object.entries(roleWeights).reduce(
      (acc, [key, value]) => {
    ...
    const changedItems: ConfirmItem[] = roles
      .filter(
        (role) =>
          ...
          parseFloat(roleEditValues[role.id] || "0") !==
            parseFloat(roleOriginalValues[role.id] || "0"),
      )
      .map((role) => ({
        label: role.title,
  ...
  const handleTicketEditStart = () => {
    const vals = ticketTypes.reduce(
      (acc, type) => {
        acc[type.id] =
          ticketTypeWeights[type.id] !== undefined
            ? ticketTypeWeights[type.id].toString()
            : "0";
        return acc;
      },
      {} as Record<string, string>,
    ...
    const changedItems: ConfirmItem[] = ticketTypes
      .filter(
        (type) =>
          ...
          parseFloat(ticketEditValues[type.id] || "0") !==
            parseFloat(ticketOriginalValues[type.id] || "0"),
      )
      .map((type) => ({
        label: type.label,
  ...
  const handleConfirmedSave = async () => {
    ...
    try {
      ...
      if (type === "role") {
        ...
        setFormulas((prev) =>
          prev.map((f, i) =>
            i === selectedFormulaIndex ? { ...f, value: items[0].newValue } : f,
          ),
        );
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-10">
          ...
          <p className="text-gray-500">
            Configure KPI calculation formulas and weights for different roles
            and ticket types
          </p>
        </div>
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div>
              ...
              <p className="text-xs text-blue-600">
                Required to load configuration
              </p>
            </div>
          </div>
        ...
        {!selectedMonth && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-gray-500 text-base font-medium">
              Please select a month above to load the configuration.
            </p>
          </div>
        )}
            ...
            <div className="mb-12">
              <div className="mb-6 flex items-center justify-between">
                ...
                <button
                  ...
                  disabled={
                    formulaSaving ||
                    formula === formulas[selectedFormulaIndex]?.value
                  }
              ...
              <div className="bg-gray-50 rounded-xl p-6 border border-gray-200 mb-6">
                <div className="mb-4">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      handleFormulaSelect(parseInt(e.target.value))
                    }
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
                  <h3 className="font-semibold text-gray-900 text-sm mb-4">
                    Variables Reference (Click to insert)
                  </h3>
                        ...
                        <td className="px-6 py-4">
                          {roleEditMode ? (
                            ...
                            <span className="text-sm font-semibold text-gray-900">
                              {(roleWeights[role.id] || 0).toFixed(1)}
                            </span>
                          )}
                        </td>
                      </tr>
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          {ticketEditMode ? (
                            ...
                            <span className="text-sm font-semibold text-gray-900">
                              {ticketTypeWeights[type.id] != null
                                ? ticketTypeWeights[type.id].toFixed(1)
                                : "-"}
                            </span>
                          )}
                        </td>
      ...
      {confirmModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div
            ...
            onClick={() =>
              !saving && setConfirmModal((prev) => ({ ...prev, open: false }))
            }
          />
          <div className="relative z-10 w-full max-w-md mx-4 bg-white rounded-2xl shadow-2xl border border-gray-200 overflow-hidden">
            ...
            <div className="px-6 py-5 border-b border-gray-200 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  {confirmModal.items.length} item
                  {confirmModal.items.length > 1 ? "s" : ""} will be updated
                </p>
              </div>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left border-b border-gray-200">
                      ...
                      <th className="pb-2 font-semibold text-gray-600 text-center">
                        Old
                      </th>
                      <th className="pb-2 font-semibold text-gray-600 text-center">
                        New
